...
VMAT.GMM.Exact <- function(theta,data,TT="PS",Var.Type="Eff"){
...
SPC.PS <- function(Y.Origin,Trt,
  ...
  VMAT.Current <- function(theta,data){
    VMAT.GMM.Exact(theta, data , TT="PS", Var.Type=Var.Type )
  }
...
SPC.PS.Sensitivity <- function(Y.Origin,Trt,
  ...
  VMAT.Current <- function(theta,data){
    VMAT.GMM.Exact(theta, data , TT="PS.Sensitivity", Var.Type=Var.Type )
  }
  MEAT.Current <- function(theta,data){
  ...
  COEF <- glm(Trt~0+Y,data=data,family="binomial",
              offset=Ptb)$coefficient
  penalty.vec <- rep(0,2+length(COEF))
  penalty.vec[2+which(apply(Y,2,mean)!=1)] <- sqrt(penalty) 
...
SPC.OR <- function(Y.Origin,Trt,
  ...
  VMAT.Current <- function(theta,data){
    VMAT.GMM.Exact(theta, data , TT="OR", Var.Type=Var.Type )
  }
  MEAT.Current <- function(theta,data){
...
SPC.DR <- function(Y.Origin,Trt,
  ...
  VMAT.Current <- function(theta,data){
    VMAT.GMM.Exact(theta, data , TT="DR", Var.Type=Var.Type )
  }
  MEAT.Current <- function(theta,data){
  ...
  penalty.vec[2+which(apply(Ymat.PS,2,mean)!=1)] <- sqrt(penalty) 
  ...
  PS.PS <- optim(par=COEF.PS,fn=GRID.PS)$par
  PS.OR <- optim(par=COEF.OR,fn=GRID.OR)$par
